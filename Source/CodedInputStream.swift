...
//            let byPointer = UnsafeMutablePointerUInt8From(data: bytes)
            _ = buffer.withUnsafeMutableBytes({ (pointer) -> Bool in
                memcpy(&bytes, pointer.baseAddress! + bufferPos, pos)
                return true
            })
            ...
            while size - pos > bufferSize {
                _ = bytes.withUnsafeMutableBytes({ (pointer) -> Bool in
                    memcpy(pointer.baseAddress! + pos, &buffer, bufferSize)
                    return true
                })
                pos += bufferSize
                bufferPos = bufferSize
            ...
            _ = bytes.withUnsafeMutableBytes({ (pointer) -> Bool in
                memcpy(pointer.baseAddress! + pos, &buffer, size - pos)
                return true
            })
            bufferPos = size - pos
...
//            let byPointer =  UnsafeMutablePointerUInt8From(data: bytes)
            ...
            _ = buffer.withUnsafeMutableBytes { (pointer) -> Bool in
                memcpy(&bytes, pointer.baseAddress! + originalBufferPos, pos)
                return true
            }
...
//                let chPointer =  UnsafeMutablePointerUInt8From(data: chunk)
                _ = bytes.withUnsafeMutableBytes({ (pointer) -> Bool in
                    memcpy(pointer.baseAddress! + pos, chunk, chunk.count)
                    return true
                })
                pos += chunk.count
            }
