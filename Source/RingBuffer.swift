...
//        let pointer = UnsafeMutablePointerUInt8From(data: buffer)
        if position >= tail {
            ...
            _ = buffer.withUnsafeMutableBytes { (bufferPointer) -> Bool in
                _ = inputs.withUnsafeMutableBytes { (inputsPointer) -> Bool in
                    memcpy(bufferPointer.baseAddress! + Int(position), inputsPointer.baseAddress! + Int(aOffset), Int(totalWritten))
                    return true
                }
                return true
            }
...
//            input.copyBytes(to: pointer + position, from: aOffset..<aOffset + totalWritten)
        ...
        _ = buffer.withUnsafeMutableBytes { (bufferPointer) -> Bool in
            _ = inputs.withUnsafeMutableBytes { (inputsPointer) -> Bool in
                memcpy(bufferPointer.baseAddress! + position, inputsPointer.baseAddress! + Int(aOffset), Int(written))
                return true
            }
            return true
        }
